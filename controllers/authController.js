...
exports.supervisorSignup = async (req, res) => {
  ...
  try {
    ...
    if(user) return res.status(400).json('User alredy exist')
      user = new Supervisor({ name, email, password, role, prefix, gender, department, phone });
      await user.save();
      ...
      res.status(201).json({ msg: 'User Sign up successfully', token, user: _.omit(user.toObject(), ['password']) });
  } catch (err) {
...
exports.researcherSignup = async (req, res) => {
  ...
  try {
    ...
    if(user) return res.status(400).json('User alredy exist')
      user = new Researcher({ name, email, password, role, gender, department, matric, phone, topic, season : seasonID._id, supervisor });
      await user.save();
